...
from ..models import FileNode, FunctionNode, ClassNode, NodeType, Language, FunctionType, CallInfo, ContentType
...
class PythonAnalyzer(LanguageAnalyzer):
    ...
    async def analyze_file(self, file_path: str) -> Optional[FileNode]:
        ...
        try:
            ...
            return FileNode(
                ...
                file_path=os.path.normpath(os.path.relpath(file_path, self.project_dir)),
                language=Language.PYTHON,
                functions=functions,
                classes=classes,
                imports=list(imports_map.values()),
                summary=await self._generate_file_summary(content, functions, classes)
            )
    ...
    async def _generate_file_summary(self, content: str, functions: List[FunctionNode], classes: List[ClassNode]) -> str:
        """使用 LLM 生成文件摘要"""
        # 判断文件规模大小，如果<1000行，直接用文件内容做诶Content生成摘要；如果>2000行，则用文件内的Class、Function摘要生成文件摘要
        if len(content) > 1000:
            content = ""

            content += f"文件包含如下类定义：\n"
            for cls in classes:
                content += f"    - {cls.name}:{cls.summary}\n"
                for method in cls.methods:
                    content += f"        - {method.name}:{method.summary}\n"
            
            content += f"文件包含如下函数定义：\n"
            for func in functions:
                content += f"    - {func.name}:{func.summary}\n"
    ...
    async def analyze_function(self, node, imports_map: dict) -> FunctionNode:
        """分析Python函数"""
    ...
    async def analyze_class(self, node: ast.ClassDef, imports_map: dict) -> ClassNode:
        """分析Python类"""
        source_code = ast.unparse(node)
        ...
        # 确定类的类型
        node_type = NodeType.CLASS  # 默认为普通类
        # 检查是否是接口/抽象类
        ...
        class_node = ClassNode(
            ...
            source_code=source_code,
            summary=await LLMSummary.llm_summarize(source_code, ContentType.CLASS),
            methods=methods,
        ...
        return class_node, class_methods

    def _get_base_classes(self, node: ast.ClassDef, imports_map: dict, module_path: str) -> List[ClassNode]:
        """获取类的父类列表
    ...
    def _get_module_path(self) -> str:
        ...
        Examples:
            ...
            返回: "app.models.user"
        """
        return os.path.splitext(os.path.relpath(self.file_path, self.project_dir))[0].replace(os.sep, '.') 