...
class Neo4jClient:
    ...
    def save_project(self, project_id: str, name: str, root_path: str):
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in save_project: {str(e)}, project_id: {project_id}, name: {name}")
                logger.error(f"Error type: {type(e)}")
                raise
    def save_folder_node(self, project_id: str, folder_node: FolderNode, parent_folder_node: FolderNode = None):
        """保存完整的文件夹结构
        1. 文件夹节点
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in save_folder_node: {str(e)}, folder_node: {folder_node}")
                logger.error(f"Error type: {type(e)}")
                raise
    ...
    def _update_folder_contains_file(self, project_id: str, folder_node: FolderInfo, file_node: FileInfo):
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in update_folder_contains_file: {str(e)}, folder_node: {folder_node}, file_node: {file_node}")
                logger.error(f"Error type: {type(e)}")
                raise
                ...
                if file_node.classes:
                    for class_node in file_node.classes:
                        if isinstance(class_node, ClassInfo):
                            ...
                            print(f"Warning: Invalid class node type: {type(class_node)}")
                ...
                if file_node.functions:
                    for func_node in file_node.functions:
                        if isinstance(func_node, FunctionInfo):
                            ...
                            print(f"Warning: Invalid function node type: {type(func_node)}")

            except Exception as e:
                logger.error(f"Error in save_file_node: {str(e)}")
                logger.error(f"Error type: {type(e)}")
                logger.error(f"File node: {file_node}")
                raise
                ...
                if class_node.methods:
                    for method_node in class_node.methods:
                        if isinstance(method_node, FunctionInfo):
                            ...
                            print(f"Warning: Invalid method node type: {type(method_node)}")    
                
            except Exception as e:
                logger.error(f"Error in save_class_node: {str(e)}")
                logger.error(f"Error type: {type(e)}")
                logger.error(f"File path: {file_node.file_path}")
                logger.error(f"Class node: {class_node}")
                raise
    ...
    def _save_class_inheritance(self, project_id: str, class_node: ClassInfo):
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in _save_class_inheritance: {str(e)}")
                logger.error(f"Error type: {type(e)}")
                logger.error(f"Class node: {class_node}")
                raise
    ...
    def save_function_node(self, project_id: str, file_node: FileInfo, function_node: FunctionInfo):
        ...
        #print(f"save_function_node: {function_node.full_name}")
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in save_function_node: {str(e)}, function_node: {function_node}")
                logger.error(f"Error type: {type(e)}")
                raise
    ...
    def save_method_node(self, project_id: str, class_node: ClassInfo, function_node: FunctionInfo):
        ...
        """
        #print(f"save_method_node: {method_node.full_name}")
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in save_method_node: {str(e)}, function_node: {function_node}")
                logger.error(f"Error type: {type(e)}")
                raise
    ...
    def _save_function_calls(self, project_id: str, function_node: FunctionInfo, call_info: CallInfo):
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in _save_function_calls: {str(e)}, function_node: {function_node}, call_info: {call_info}")
                logger.error(f"Error type: {type(e)}")
                raise
    ...
    def delete_stale_nodes(self, project_id: str, before_timestamp: str):
        ...
        with self.driver.session() as session:
            try:
                ...
                logger.error(f"Error in delete_stale_nodes: {str(e)}, project_id: {project_id}, before_timestamp: {before_timestamp}")
                logger.error(f"Error type: {type(e)}")
                raise
        ...
        with self.driver.session() as session:
            with session.begin_transaction() as tx:
                try:
                    ...
                    tx.rollback()
                    logger.error(f"Error in delete_file_nodes: {str(e)}, project_id: {project_id}, file_path: {file_path}")
                    logger.error(f"Error type: {type(e)}")
                    raise e
        ...
        with self.driver.session() as session:
            with session.begin_transaction() as tx:
                try:
                    ...
                    tx.rollback()
                    logger.error(f"Error in delete_folder_nodes: {str(e)}, project_id: {project_id}, folder_path: {folder_path}")
                    logger.error(f"Error type: {type(e)}")
                    raise e
